...
class Object {
  ...
  // physics state kept in own buffer of states for better cpu cache utilization
  // at update may change between frames (when objects are deleted and 'phy_'
  // relocated)
  PhysicsState *phy_{};
  ...
  // normals of bounding shape rotated to the world coordinates (not normalized
  // if scale!=1)
...
public:
  Object(const TypeBits type_bits, const TypeBits type_bits_collision_mask,
         const ObjectDef &def, const Scale scale, const Scalar bounding_radius,
  ...
  virtual auto draw(Bitmap8b &dsp) -> void {
    ...
    if (enable::draw_polygons) {
      if (def_.indexes_size == 1) {
        ...
        dsp.draw_polygon(points_world_.get(), def_.indexes_size, def_.indexes,
                         color_);
      }
    }
...
private:
    ...
    for (Object **it1 = ls_all; it1 < ls_all_pos - 1; ++it1) {
      for (Object **it2 = it1 + 1; it2 < ls_all_pos; ++it2) {
        ...
        const bool o1_check_collision_with_o2 =
            o1->type_bits_collision_mask_ & o2->type_bits_ && o1->is_alive();
        const bool o2_check_collision_with_o1 =
            o2->type_bits_collision_mask_ & o1->type_bits_ && o2->is_alive();
  ...
  static auto is_any_point_in_bounding_shape(const Object &o1, const Object &o2)
      -> bool {
