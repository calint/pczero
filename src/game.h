...
public:
  ...
  static auto is_in_play_area(const Object &o) -> bool {
    ...
    const Real ymax =
        Real(Game::play_area_top_left.y + Game::play_area_dim.height());
...
public:
  ...
  ~Enemy() override { Game::enemies_alive--; }
...
public:
  ...
  Ship()
      : Object{tb_ships,
               tb_bosses | tb_bullets | tb_enemies | tb_missiles | tb_ships |
                   tb_walls,
               Game::ship_def,
               def_scale,
               def_bounding_radius,
  ...
  ~Ship() override { Game::player = nullptr; }
...
private:
  ...
  auto find_aim_vector_for_moving_target(
      const Object &target, const TimeStepSec eval_t, const TimeStepSec eval_dt,
      const Real error_margin_t, const bool draw_trajectory = false) -> Vector {
    TimeStepSec t = 0;
...
public:
  ...
  ~Boss() override { Game::boss = nullptr; }
...
[[noreturn]] auto Game::run() -> void {
  ...
  enemy_def = {
      5, 4,
      new Point[]{
          // points in model coordinates, negative Y is "forward"/"up"
          {0, 0},
          {-1, -.5},
          {-1, .5},
          {1, .5},
  ...
  while (true) {
    // clear game area
    pz_memcpy(clear_start_at_address, clear_copy_from_address,
              clear_copy_num_bytes);
